...
class TestMain:
    ...
    async def test_main_fetch_error(self):
        ...
        with patch('claude_code_docs_spa.main.ClaudeCodeFetcher') as mock_fetcher_class:
            mock_fetcher = MagicMock()
            mock_fetcher.fetch_all_documentation.side_effect = Exception("Network error")
            mock_fetcher_class.return_value = mock_fetcher
    ...
    async def test_main_install_success(self):
        ...
        with patch('claude_code_docs_spa.main.ClaudeCodeInstaller') as mock_installer_class:
            ...
            assert result["status"] == "success"
            assert "installed successfully" in result["message"].lower() or "installation completed" in result["message"].lower()
            assert "result" in result
    ...
    async def test_main_uninstall_success(self):
        ...
        with patch('claude_code_docs_spa.main.ClaudeCodeInstaller') as mock_installer_class:
            ...
            assert result["status"] == "success"
            assert "uninstalled successfully" in result["message"].lower() or "uninstallation completed" in result["message"].lower()
            assert "result" in result
